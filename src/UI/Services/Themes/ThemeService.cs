...
internal class ThemeService(IThemeProvider provider, IMessenger messenger) : IThemeService
{
	...
	{
		ThemeDescriptor descriptor = provider.Get(theme);
	...
	{
		ThemeType next = CurrentTheme is ThemeType.Default
			? ThemeType.Dark
			: ThemeType.Default;
